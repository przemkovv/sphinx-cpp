...
Sandbox::Sandbox() : logger_(make_logger(name()))
{
  ...
  project_root_path_ =
      fs::unique_path(fs::temp_directory_path() / "sphinx"s / "%%%%%%%"s);
...
void Sandbox::create_directory(const fs::path &path)
{
  ...
  if (!exists(path)) {
    throw std::invalid_argument(
        fmt::format("Cannot create directory: {}", path.string()));
  }
}

Sandbox::Sandbox(std::initializer_list<File> files) : Sandbox()
{
  add_files(files);
}

template <class C> void Sandbox::add_files(const C &files)
{
