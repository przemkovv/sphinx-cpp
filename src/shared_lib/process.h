  ...
  public:
    ...
    int_type underflow() override
    {
        ...
        if (eback() == base)
        {
            ...
            const auto area = static_cast<std::size_t>(egptr() - dest) < put_back_size_ ? static_cast<unsigned long>(egptr() - dest) : put_back_size_;
        ...
        auto bytes
            = stdout_pipe_.read(start, in_buffer_.size() - static_cast<unsigned long>(start - base));
...
class pipe_streambuf : public pipe_ostreambuf
{
  ...
  private:
    void flush() override
    {
        ...
        {
            stdin_pipe_.write(pbase(), static_cast<uint64_t>(pptr() - pbase()));
...
class process
{
  public:
    ...
    void append_arguments(InputIterator first, InputIterator last) {
        args_.insert(args_.end(), first, last);
    }
